...
packages=('dosfstools' 'ntfs-3g' 'parted' 'gdisk' "${@/$1}")

systemd-machine-id-setup

pacman-key --init; pacman-key --populate
pacman -Syu --needed --noconfirm "${packages[@]}"&& printf '\e[32m>\e[m: %s\n' 'Updated & installed packages'

printf '\e[32m>\e[m: %s\n%s\n' "Done. Arch installer environment setup; Chaged root into $1/root.x86_64" \
